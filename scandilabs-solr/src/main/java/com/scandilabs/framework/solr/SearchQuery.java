...
public class SearchQuery {
    ...
    public void addExplicitQuery(String query, String conditional) {
        String currentQuery = this.solrQuery.getQuery();
        if (currentQuery.length() > 0) {
          currentQuery+= String.format(" %s (%s)", conditional, query);
        }
        else {
          currentQuery= query;
        }
        ...
        verifyRequestMethod(currentQuery);
    }
     ...
     * 
     * if end is specified - created:[%s/DAY TO %s+1DAY/DAY] else -
     * created:[%s/DAY TO *]
     * 
     * @param start
     * @param end
    ...
    public void setCreatedDateRange(Date start, Date end) {
        ...
        if (end == null) {
            String createdRange = String.format("created:[%s/DAY TO *]",
                    formatDate(start));
            this.solrQuery.addFilterQuery(createdRange);
        } else {
            String createdRange = String.format(
                    "created:[%s/DAY TO %s+1DAY/DAY]", formatDate(start),
                    formatDate(end));
            this.solrQuery.addFilterQuery(createdRange);
        }
    }
    ...
    private String formatDate(Date d) {
        return String.format("%tFT%tTZ", d, d);
    }
